...
def arima(data):
  ...
  for t in range(len(ypred)):
    ...
    actual_list.append(actaul)
    min_list.append(min_)
    max_list.append(max_)
    obs = ypred[t]
    history.append(obs)
  
  final_df = pd.DataFrame(list(zip(actual_list,min_list,max_list)), columns=['Arima_pred','Arima_min', 'Arima_max'])
  return final_df
...
def core(data):
  ...
  arima_data=arima(data[['close_x']])
  data.drop(data.head(5).index,inplace=True)
  data=pd.concatenate([data,arima_data] ,axis = 1)
  ...
  maty= y.reshape(x.shape[0], -1)
  scalerY.fit(maty)

  maty= scalerY.transform(maty)

  dim_0 = x.shape[0] - TIME_STEPS
  dim_1 = x.shape[1]
...
def train(data):
  ...
  lstm_model.add(LSTM(20,return_sequences=True, stateful=False,batch_input_shape=(BATCH_SIZE,TIME_STEPS, x.shape[2]), dropout=0.0, recurrent_dropout=0.0,kernel_initializer='random_uniform'))
  lstm_model.add(LSTM(30, return_sequences=True,dropout=0.0))
  lstm_model.add(LSTM(20, return_sequences=True, dropout=0.0))
  lstm_model.add(LSTM(20, return_sequences=True,dropout=0.0))
  lstm_model.add(LSTM(20, return_sequences=False,dropout=0.0))
  lstm_model.add(Dense(40 ,activation='linear'))
  lstm_model.add(Dense(30,activation='linear'))
  lstm_model.add(Dense(20,activation='linear'))
  lstm_model.add(Dense(1 ,activation='linear'))
  lstm_model.compile(loss='mse', optimizer='adam', metrics=['mse'])
  lstm_model=load_model("modelclassifier_lstm.h5")
  checkpoint = ModelCheckpoint('modelclassifier_lstm.h5', monitor='loss', verbose=1, save_best_only=True, mode='min')
  callbacks_list = [checkpoint]
  lstm_model.fit(x, y, epochs=100,batch_size=BATCH_SIZE,callbacks=callbacks_list)
  lstm_model.save("modelclassifier_lstm.h5")
  clear_session()
...
#   lstm_model = Sequential()
#   lstm_model.add(LSTM(50,return_sequences=False, stateful=False,batch_input_shape=(BATCH_SIZE,TIME_STEPS, x.shape[2]), dropout=0.0, recurrent_dropout=0.0,kernel_initializer='random_uniform'))

#   lstm_model.add(Dense(1 ,activation='linear'))
#   lstm_model.compile(loss='mse', optimizer="adam")
#   #lstm_model=load_model("modelclose2.h5")
#   checkpoint = ModelCheckpoint('modelclose2.h5', monitor='loss', verbose=1, save_best_only=True, mode='min')
#   callbacks_list = [checkpoint]
#   lstm_model.fit(x, y, epochs=10, verbose=2, batch_size=BATCH_SIZE,shuffle=True ,callbacks=callbacks_list)
#   lstm_model.save("modelclose2.h5")


def pred(data):
  model=load_model("modelclassifier_lstm.h5")
  x,y,scalerY=core(data)
  ypred1=model.predict(x,batch_size=BATCH_SIZE)
  y = y.reshape(-1, 1)
